...
>
> ⚠️ **2026-08-25 更新（CSS-11）**：内层限流的**计数维度**已从客户端 IP 改为
> `X-User-Id`（header 缺失时回退到 IP），见 ROADMAP 第 16 项。本文下述两条不变量
> 讲的是**层级顺序**，与计数维度无关，因此依然成立；原文中的「per-IP 层」一律
> 改称「内层」。
...
计数桶（`CHAT_GLOBAL_RATE_LIMIT`，默认 `500/day`）。内层管「单个调用方
别刷太快」，全局层管「全站总量封顶」——换 IP 或换 `X-User-Id` 绕得过前者，
绕不过后者。
...
1. **两层限流都必须传 callable，不能传字符串字面量。**
   ...
   书写顺序无关。若全局层退化成字符串，会被提前评估，被内层拦下的请求
   照样烧全局额度。

2. **装饰器顺序 load-bearing：内层必须是靠近函数的底部装饰器。**
   两层都是 callable 时按装饰器自下而上的注册顺序评估，且 slowapi 的
   hit 是「先扣再判、失败即断」。内层在底部 → 先评估 → 它的 429 在触碰
   全局计数**之前**短路，单个狂刷的调用方烧不掉全站额度。顺序反了，一个
   调用方用被拒绝的请求就能把全站打到熔断。回归哨兵：
   `test_per_ip_429s_do_not_burn_the_global_budget` 与
   `test_per_user_429s_do_not_burn_the_global_budget`（分别覆盖回退到 IP
   和按 `X-User-Id` 两条计数路径）。

   反向代价（可接受）：请求过了内层但被全局拦下时，该调用方的分钟额度
   被白扣一次，不退款。
...
- 429 响应体两层完全一致（统一安全错误体，不泄漏限额数值）；区分哪层
  触发看服务端 WARNING 日志（`"10 per 1 minute"` = 单个调用方噪音，
...
- 数请求数不数 token：单个大请求成本远高于均值。输入体积上限（roadmap 19.5）
  已随 CSS-6（`ae68a46c`）落地，因此上界可算：单请求最大成本约 **$0.0097**
  （100,113 字符 ≈ 61,512 tokens，推导见
  [openai-spend-cap.md](../../openai-spend-cap.md) 的「上限设多少」）。默认
  `500/day` 对应的最坏情况是 **$4.85/天、约 $150/月**——高于当前 $20 的
  project 级硬上限，即硬上限才是实际生效的那道闸，本层是它前面的缓冲。
- 全局额度可被恶意打满（全站 429 到窗口结束）：这是「封顶优先于可用性」
  的有意选择——被打满的损失上界是一天的限额成本（按上面的上界即 $4.85/天），
  而不设全局层的损失上界是 OpenAI 账单。缓解手段是内层先行拦截 +
  日志告警人工介入。
