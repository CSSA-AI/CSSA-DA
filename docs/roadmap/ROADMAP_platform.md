...
> 干净的 `alembic upgrade head`。（2026-09-22，#105 起后面多了一句
> `&& python -m ops.provision_runtime_role`：迁移之后把 API 的运行时角色对齐到授权清单，
> 仍然只报一个退出码。）
>
...
### 20. 首次语料导入生产 RDS（2026-09-13 新增，v1 阻塞项）⏳ 代码与配置已落地，待上线执行

> **状态：代码与配置已落地（2026-09-22，#105），待在生产上执行一次。** 分两段：
>
> - **导入动作（2026-09-17，#111）**：2312 行入库，`/ready` 转 200，`/v1/chat` 端到端
>   能答。卡点（语料不在镜像里）的解法：S3 数据桶 + 预签名 URL，容器用 `urllib` 拉下来，
>   **不需要任何新的 IAM 权限**。那次导入跑在 API 容器里 —— 当时 API 还用主用户连库。
> - **#105 其余的完成标准（2026-09-22）**：
>   - 导入在那一刻记下 `corpus_sha256`，每条记录按键和内容回库核对：checkpoint 说「已完成」
>     而库被重建、撞上同名的另一个库、或库里是另一版语料时，命令失败。日志最后一行就是导入
>     报告，其中 `knowledge_base_rows` 与 `/ready` 用同一条 SQL 数。
>   - 运行时身份与迁移身份分开：迁移任务每次部署都跑 `ops/provision_runtime_role.py`，把
>     `cssa_app` 的权限对齐到清单并验证它**实际**拥有的权限；API 任务改用它连库，不再拿主
>     用户密钥。
...
> 一组同一篇出现 7 次）。`(link, question_text)` 唯一索引拦不住 —— 同样的内容重发一次就是
> 一个新 URL。后果不是分数难看，而是 **top-5 可能返回同一篇的五个副本**，模型拿到的上下文
