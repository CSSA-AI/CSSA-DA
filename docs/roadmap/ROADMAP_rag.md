...
>
> **召回低的直接原因:embedding 只看到了每篇文章的开头。** 线上 embedding 模型
> `paraphrase-multilingual-MiniLM-L12-v2` 的 `max_seq_length` 是 **128**,而
> [`build_embedding_text`](../../pipelines/embedding/knowledge_base_text.py) 把「标题 + 整篇正文」
> 当一段文本编码(中位数 1156 token)。线上 2312 行里 **95% 被截断**,向量实际代表的是标题加
> 正文开头约 159 个字 —— 正文的 8.6%。问题只要不落在文章开头,检索器从原理上就找不到。
>
> 同一批题上换文档表示方式的对照(模型和查询都不变,top-30 召回):
>
> | 文档表示 | 中文提问 → 文章 | 中文提问 → 英文 handbook | 已有 GPT 问题 |
> |---|---|---|---|
> | 现状:整篇截断 128 | 36.8% | 49.6% | 51.4% |
> | 截断放宽到 512 | **7.4%** | 7.8% | 70.8% |
> | 分块:正文切 ~110 token 窗口(步长 80)、每块带标题、文档取最高分 | **61.1%** | 62.5% | 75.0% |
>
> - **分块是这里最便宜的一步**:不换模型、不改向量维度,中文召回 +24 个点,top-5 命中 19.5% → 40.5%。
>   代价是向量行数约 ×15.5,以及入库 schema 与检索去重要跟着改
> - ⚠️ **陷阱:不要只把 `max_seq_length` 调大。** 模型只在 128 上训练过,均值池化被长文本稀释,
>   中文召回反而崩到 7.4%。「截断」这个词会让人想顺手改参数,那样会把线上搞坏
> - 这与 [ROADMAP_data](ROADMAP_data.md#这个前提决定了什么)「chunking 回到正常优先级」的判断
>   一致,补上的是数字:**现在线上就已经是 95% 的行被截断,不是将来文档源变多才出现的问题**
